...
def Lavi_Dish_Nutrition_Insert_Update(doc, method):
    # Create a new document for Lavi_Dish_Ingredient_Nutrition
    new_ingredient_nutrition = frappe.get_doc({
        "doctype": "Lavi_Dish_Ingredient_Nutrition",
        "dish": doc.dish_name,
    ...
    # Initialize sums for nutrition values
    energy_kcal = 0.0
    carbohydrates = 0.0
    sugars = 0.0
    protein = 0.0
    ...
    for ingredient in doc.ingredients:
        ...
        # Sum values for Lavi_Dish_Nutrition
        energy_kcal += float(nutrition.energy_kcal or 0)
        carbohydrates += float(nutrition.carbohydrates or 0)
        sugars += float(nutrition.sugars or 0)
        protein += float(nutrition.protein or 0)
        fibre += float(nutrition.fibre or 0)
        total_fat += float(nutrition.total_fat or 0)
        sat_fat += float(nutrition.sat_fat or 0)
        ...
        # Sum values for Lavi_Dish_Nutrition
        cholestrol_mg += float(nutrition.cholestrol_mg or 0)
        sodium_mg += float(nutrition.sodium_mg or 0)
        mono_unsat_fat += float(nutrition.mono_unsat_fat or 0)
        poly_unsat_fat += float(nutrition.poly_unsat_fat or 0)
        calcium_mg += float(nutrition.calcium_mg or 0)
        iron_mg += float(nutrition.iron_mg or 0)
    ...
    # Fetch or create the Lavi_Dish_Nutrition document
    dish_nutrition_name = frappe.db.exists("Lavi_Dish_Nutrition", {"dish": doc.dish_name})
    if dish_nutrition_name:
        dish_nutrition = frappe.get_doc("Lavi_Dish_Nutrition", dish_nutrition_name)
    else:
        dish_nutrition = frappe.get_doc({
...
def create_and_update_meal_nutrition(doc, method):
    # Create a new document for Lavi_Meal_Dish_Nutrition
    new_dish_nutrition = frappe.get_doc({
        "doctype": "Lavi_Meal_Dish_Nutrition",
        "select_meal": doc.name1,
        "food_group": doc.food_group
    ...
    for dishs in doc.dish_information:
        ...
        # Sum values for the first child table
        energy_kcal += float(nutrition.energy_kcal or 0)
        carbohydrates += float(nutrition.carbohydrates or 0)
        sugars += float(nutrition.sugars or 0)
        protein += float(nutrition.protein or 0)
        ...
        # Sum values for the second child table
        cholestrol_mg += float(nutrition.cholestrol_mg or 0)
        sodium_mg += float(nutrition.sodium_mg or 0)
        mono_unsat_fat += float(nutrition.mono_unsat_fat or 0)
        poly_unsat_fat += float(nutrition.poly_unsat_fat or 0)
    ...
    # Fetch or create the Lavi_meal_Nutrition document
    meal_nutrition_name = frappe.db.exists("Lavi_Meal_Nutrition", {"meal_name": doc.name1})
    if meal_nutrition_name:
        meal_nutrition = frappe.get_doc("Lavi_Meal_Nutrition", meal_nutrition_name)
    else:
        meal_nutrition = frappe.get_doc({
